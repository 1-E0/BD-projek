...
public class PesananDAO {
    ...
    public void simpanPesananLengkap(Pesanan pesanan, List<DetailPesanan> detailList, Pembayaran pembayaran) throws SQLException {
        String sqlPesanan = "INSERT INTO pesanan (id_pelanggan, total_harga_pesanan, status_pembayaran, alamat_tujuan) VALUES (?, ?, ?, ?) RETURNING id_pesanan";
        String sqlDetail = "INSERT INTO detail_pesanan (id_pesanan, id_menu, kuantitas, harga_produk) VALUES (?, ?, ?, ?)";
        String sqlPembayaran = "INSERT INTO pembayaran (id_pesanan, id_metode) VALUES (?, ?)";

        try {
            conn.setAutoCommit(false); // Mulai Transaksi

            // 1. Simpan Pesanan, dapatkan ID baru
            int idPesananBaru;
            try (PreparedStatement pstmtPesanan = conn.prepareStatement(sqlPesanan)) {
                pstmtPesanan.setInt(1, pesanan.getIdPelanggan());
                pstmtPesanan.setDouble(2, pesanan.getTotalHargaPesanan());
            ...
            conn.commit(); // Sukses, simpan semua perubahan
            System.out.println("Transaksi pesanan lengkap berhasil di-commit.");

        } catch (SQLException e) {
            System.err.println("Transaksi GAGAL, melakukan rollback.");
            conn.rollback(); // Jika ada error, batalkan semua
            throw e; // Lemparkan error agar bisa ditangani di controller
        } finally {
            conn.setAutoCommit(true); // Kembalikan ke state normal
        }
    }
